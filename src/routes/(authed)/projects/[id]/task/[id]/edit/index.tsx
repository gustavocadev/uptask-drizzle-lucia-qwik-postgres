...
export const useActionUpdateTask = routeAction$(
	async (values, request) => {
		const parseDateToUTC = dateFns.parse(values.dueDate, 'yyyy-MM-dd', new Date())
...
export const useLoaderTask = routeLoader$(async ({ params }) => {
	...
	const inputDateFormat = dateFns.format(new Date(task.deliveryDate), 'yyyy-MM-dd')
		...
		<>
			<div class="sm:flex sm:items-start">
				<div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
					<h3 class="text-4xl leading-6 font-bold text-gray-900">Crear Tarea</h3>
					<Form class="mt-10" action={actionUpdateTask}>
						...
						<div class="mb-5">
							<label for="due-date" class="block text-gray-700 text-sm font-bold mb-2">
								Fecha de entrega
							</label>
							<input
								type="date"
						...
						<input type="hidden" name="taskId" value={loaderTask.value.task?.id} />
						<input type="hidden" name="projectId" value={loaderTask.value.task?.projectId} />
						<button
							type="submit"
