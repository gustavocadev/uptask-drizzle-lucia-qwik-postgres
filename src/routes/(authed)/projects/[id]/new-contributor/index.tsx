...
import { Form, routeLoader$, z, zod$, routeAction$ } from '@builder.io/qwik-city'
		...
		<>
			...
			<div class="mt-10 flex justify-center">
				<Form
					...
					<div class="mb-5">
						<label class="text-gray-700 uppercase font-bold text-sm" for="email">
							Email Colaborador
						</label>
						<input
							type="email"
			...
			{actionSearchUsers.value?.user && (
				<div class="flex justify-center mt-10">
					<div class="bg-white py-10 px-5 md:w-1/2 rounded-lg shadow w-full">
						...
						<Form class="flex justify-between items-center" action={actionAddContributor}>
							<p>{actionSearchUsers.value.user.name ?? ''}</p>
							<input type="hidden" name="projectId" value={loaderProject.value.project?.id} />
							<input type="hidden" name="userId" value={actionSearchUsers.value.user.id} />
							<button
								type="submit"
